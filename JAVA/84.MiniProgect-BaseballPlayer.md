...
- 선수 방출 시, Table에서 바로 삭제하지 않고 `bye` field를 `1`로 바꿔 선수 조회 시 검색 되지 않게 할거다.
...
- [DAO 코드 보러가기](https://github.com/6suk/JavaLecture/blob/master/src/mysql/baseballPlayer/DAO.java)

  <img src="./img/Untitled 1.png">

> 기존 과제에서는 1.선수 목록, 2.선수 등록, 3.선수정보 수정, 4.선수 방출 요렇게 4개만 만들면 됐지만, 방출된 선수들 다시 데리고 오고 싶으면 어쩌죠..? 😢 난 그래서 **5. 선수 재입단**도 넣어 보았다.
...
- 입력은 `BufferReader`로 받고 모든 메소드에서 `br.readline()` 로 편하게 쓸 수 있도록
  `public static`객체를 생성했다. ( DAO도 마찬가지로! )
...
- 방출 후에는 **“[선수 이름]가 방출되었습니다.”** 라는 메세지가 뜨도록 DAO `byePlayer()` 메소드에 설정해놨다.
